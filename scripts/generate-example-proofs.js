...
/**
 * ⚠️ EXACTLY like scripts/generate-example-proofs.js:
 ...
 */
function splitPubkeyTo128_LIMBS_MATCH_SCRIPT(pk: PublicKey): { hi: bigint; lo: bigint } {
  const be = pk.toBytes();               // canonical order (32 bytes)
  const lo = bigFromBytesLE(be.slice(0, 16));  // interpret first 16 bytes as LE
  const hi = bigFromBytesLE(be.slice(16, 32)); // interpret last 16 bytes as LE
...
describe("E2E: withdraw via client recipient ATA", () => {
  ...
  it("prepares, (optionally) proves, and submits a withdraw to client ATA", async () => {
    ...
    const submitBody: any = {
      ...
      // client-specified recipient (owner + ATA) + **limbs matching the script**
